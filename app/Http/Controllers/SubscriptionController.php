...
class SubscriptionController extends Controller
{
    ...
    public function show()
    {
        ...
        //return view('subscribe')->with(['plans' => Plan::all(), 'paymentPlatforms' => $paymentPlatforms,]);
        return (['plans' => Plan::all(), 'paymentPlatforms' => $paymentPlatforms,]);
    }
    ...
    public function approval(Request $request)
    {
        ...
        if (session()->has('subscriptionPlatformId')) {
            ...
            if ($paymentPlatform->validateSubscription($request)) {
                ...
                return redirect()
                    ->route('home')
                    ->withSuccess(['payment' => "Gracias, {$user->name}. Tu suscripción: {$plan->slug}. Enjoy it! ;)"]);
            }
        }

        return redirect()
            ->route('subscribe.show')
            ->withErrors('No pudimos validar tu suscripción. Intenta de nuevo, por favor.');
    }
    ...
    public function cancelled()
    {
        return redirect()
            ->route('subscribe.show')
            ->withErrors('Cancelaste el Pago. Vuelve cuando estés listo :)');
    }
}
